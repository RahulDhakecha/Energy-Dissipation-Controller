...
% q1=0.9924;                              %quaternions corresponding to upside down position
% q2=0.0868;
% q3=-0.0867;
% q4=0.0084;

Wo0=[0;0;0];            %initial angular velocity of Control CS with respect to Orbit CS(in rad/s)
...
for t0=1:1:20000                                          %for loop for continuous evaluation of system for one complete orbital motion(time=100*60 seconds)
...
%%Online position vectors of Oersted Satellite were xactly opposite to the
%%vectors obtained from sgp4. So sign of 'ecf' vector is changed
...
if(ckoz<0)
    if(flag1==0)
e_gain=1500000;                                         %constant gain for time less than 1000 seconds
m=((100000000*(cross(Wo0,B)))-(e_gain*(Q_matrix*B)))                                           %magnetic moment
    else
        m=[0;0;0]
    end
else
        flag1=1;
    e_gain=300000+1400000*csch(t0/1000);                  %time varying hyperbolic gain as given in thesis
    m=((100000000*(cross(Wo0,B)))-(e_gain*(Q_matrix*B)))                                           %magnetic moment
end
